...
class CasRel(nn.Module):
    def __init__(self, config):
        ...
        # self.conv1 = GCNConv(self.config.bert_dim,16)
        # self.conv2 = GCNConv(16,self.config.num_relations)

        #conv的输入特征，是图卷积部分的数据输入特征，与其他模型输出无关！例如cora数据集，1433维特征，故输入特征为1433维！
    ...
    def get_gcn_input(self, input_feature, adj):
        return input_feature, adj

    #主实体识别，Linear+sigmoid
    def get_subs(self, encoded_text, input_feature, adj):
        ...
        output = torch.reshape(encoded_text, (-1, 16, 16, 768))
        ...
        # adj -> ?
        # 原 output -> 8,256,768
        # 经过GCN处理的output -> 8,256,768
        # surport = self.conv1(input_feature, adj)
        # gcn_output = self.conv2(surport, adj)



        #################################################??????????????????????????????????##########################
        gcn_output = self.conv1(output, adj)
        # gcn_output = self.conv1(input_feature, adj)

        # print(gcn_output.shape) #[1, 64, 768]
        # print(gcn_output)


        fused_features = torch.cat((output, gcn_output), dim=2)
        # fused_features = output + gcn_output
        # print(fused_features.shape)
        # print(fused_features)

        pred_sub_heads = torch.sigmoid(self.sub_heads_linear(fused_features))
        pred_sub_tails = torch.sigmoid(self.sub_tails_linear(fused_features))
    ...
    def forward(self, token_ids, mask, sub_head, sub_tail, adj, input_feature):
        #这一步，进行bert编码
        encoded_text = self.get_encoded_text(token_ids, mask)
        #get_subs，对编码层进行一个线性层+一个Sigmoid激活函数进行判断，判断每个token是不是头实体的开始或结束
        pred_sub_heads, pred_sub_tails = self.get_subs(encoded_text, input_feature, adj)
