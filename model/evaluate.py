...
def metric(data_iter, rel_vocab, config, model, output=True, h_bar=0.5, t_bar=0.5):
    ...
    for batch_x, batch_y in tqdm(data_iter):
        with torch.no_grad():
            ...
            mask = batch_x['mask']
            input_feature = batch_x['input_feature']
            adj = batch_x['adj']
            encoded_text = model.get_encoded_text(token_ids, mask)
            input_feature, adj = model.get_gcn_input(input_feature, adj)
            pred_sub_heads, pred_sub_tails = model.get_subs(encoded_text, input_feature, adj)
