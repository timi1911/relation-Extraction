...
######注意，这里理解错误，图卷积网络模型需要的edge_index输入格式，是[2,N]，表示连接关系，可以直接用依存关系套入。
    ...
    adjacency_matrix = np.zeros((64, 64), dtype=np.float)
    ...
    for i in range(num_edges):
        ...
        # 确保节点编号在有效范围内
        assert 0 <= source < 64, f"Source node number {source} out of range."
        assert 0 <= target < 64, f"Target node number {target} out of range."
...
#########扩容256个节点的特征
        ...
        for i in range(64 - len(input_feature)):
